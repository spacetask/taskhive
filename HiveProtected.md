...
The rules should be included as an appendix (Appendix I) for ease of reading.
...
It is recommended that, like the interpretation rules, the arbitration rules be directly incorporated into the contracts as a mandatory clause which cannot be modified by users. These rules should use the UNCITRAL rules as a base, with adaptations to make them suitable for HiveProtected contracts.

The rules should be included as an appendix (Appendix II) for ease of reading.
...
> 
> (6) The Arbitrator's fees shall be `FIXED AMOUNT OR PERCENTAGE` per settled dispute, controversy or claim.
>
